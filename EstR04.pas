...
type
  ...
  TfmEstR04 = class(TfmPadrao)
    ...
    Colunas1: TMenuItem;
    EstR03SEQSDF: TIntegerField;
    EstR03OBSSDF: TStringField;
    grR04SEQSDF: TdxDBGridColumn;
    grR04OBSSDF: TdxDBGridColumn;
...
begin
  ...
  sBase := ' Select EstHis.DteHis,' +
    ...
    '        EstHis.QtMovi,' +
    '        EstPro.CodUns,' +
    '        TextoOcor(EstHis.CodOpe,' + QuotedStr('-') + ',EstOpe.NomOpe,' + QuotedStr('') + ',' + QuotedStr('') + ',' + QuotedStr('') + ',' + QuotedStr('') +
    ') as Operacao,' +
    '        EstHis.CodClp || _UNICODE_FSS ' + QuotedStr('-') + ' || EstHis.CodGru || _UNICODE_FSS ' + QuotedStr('.') + ' || EstHis.CodSub || _UNICODE_FSS ' +
    QuotedStr('.') + ' || EstHis.CodPro as Item' +
    ' From EstHis ' +
    ...
    ' LEFT JOIN ESTSDF ON (ESTSDF.anosdf = EstHis.anosdf and EstSdf.seqsdf = EstHis.seqsdf)';
...
begin
  ...
  if Trim(fLimpaStr(EdPsqDtInic.Text)) <> '' then
    sFiltro := ' Where EstHis.DteHis >= ' + QuotedStr(fDateToSQL(EdPsqDtInic.Date));
