...
type
  ...
  TfmEstR11 = class(TfmPadrao)
    ...
    EstR11CLSIPI: TStringField;
    CLSIPI: TdxDBGridColumn;
    EstR11CEST: TStringField;
    grR11Column8: TdxDBGridColumn;
...
begin
  ...
  if Trim(fLimpaStr(EdDteEst.Text)) = '' then
    fmsgErro('Campo de Preenchimento Obrigatorio não Informado.', EdDteEst);
  ...
  case rgValor.Itemindex of
    ...
    4: sValor := 'Custo Ponderado';
    5: sValor := 'Custo Ponderado (G)';
    6: sValor := 'Custo Referencial';
    //Removido Custo referencial para Biasi. Também não há sentido ter isto já
    //que não tem ligação com movimentação do estoque.

    (*
     ** Na data 12/05/2014 a Propria Biasi(Leandro), pediu para incluir novamente
     **o custo referencial na pesquisa
     *)
  end;
  ...
  begin
    ...
    Text := ' Insert Into EstR11(SeqArq,CodEmp,CodClp,CodGru,CodSub,CodPro,DteEst,TipCst)' +
      ...
      ' From EstIte '+
      ' LEFT JOIN GerEmp ON (EstIte.CodEmp = GerEmp.CodEmp)' + sFiltro;
  ...
  begin
    ...
    Text := ' Select EstR11.*,' +
      ...
      ' EstPro.CodNcm ClsIPI, EstPro.CEST' +
      ...
      ' LEFT JOIN ESTIPI ON EstIpi.codipi = EstPro.ipisai AND EstIpi.tipipi = ''Saida'' ' +
    ...
    if debughook > 0 then
    Clipboard.asText := EstR11.SQL.Text;
