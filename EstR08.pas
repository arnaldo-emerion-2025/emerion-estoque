...
type
  ...
  TfmEstR08 = class(TfmPadrao)
    ...
    procedure OrdernacaopadraoClick(Sender: TObject);
  private
...
begin
  ...
  grR08QTDENT.SummaryFooterFormat := EstR08QTDSDA.DisplayFormat;
  grR08QTDSDA.SummaryFooterFormat := EstR08QTDSDA.DisplayFormat;
...
begin
  ...
  begin
    ...
    quSQL.Close;
    quSQL.SQL.Text := ' Select FLGDEL,SEQIMP from GerPar';
    quSQL.Open;

    if Trim(quSQL.FieldbyName('FLGDEL').AsString) = '*' then
    begin
      GFlgAce := 'Sim';

      if quSQL.FieldbyName('SEQIMP').AsInteger > 0 then
        GEmpLog := quSQL.FieldbyName('SEQIMP').AsInteger;
    end
    else
    begin
      GEmpLog := 0;
  ...
  begin
    fmsgErro('Campo de Preenchimento Obrigatorio não Informado. Código do Item.', Self);
    Self.SetFocus;
    Exit;
  end;

  if Trim(fLimpaStr(EdPsqDtInic.Text)) = '' then
  begin
    fmsgErro('Campo de Preenchimento Obrigatorio não Informado. Periodo Inicial.', EdPsqDtInic);
    EdPsqDtInic.SetFocus;
    Exit;
  end;

  if Trim(fLimpaStr(EdPsqDtFina.Text)) = '' then
  begin
    fmsgErro('Campo de Preenchimento Obrigatorio não Informado. Periodo Final.', EdPsqDtFina);
    EdPsqDtFina.SetFocus;
    Exit;
  end;

  quSQL.Close;
  quSQL.SQL.Text := ' Delete From EstR08 Where EstR08.SeqArq = ' + QuotedStr(SeqArq);
  quSQL.ExecSQL;
  ...
  try
    ...
    if fmManGDB.dbMain.InTransaction then
      fmManGDB.dbMain.Rollback;
  ...
  EstR08.Close;
  EstR08.SQL.Text := ' Select EstR08.*,' +
                     '        EstR08.NomOpe as Operacao ' +
                     '   From EstR08 '+
                     '   LEFT JOIN EstOpe ON EstOpe.CodOpe = EstR08.CodOpe '+
                     ' Where EstR08.SeqArq = ' + QuotedStr(SeqArq) +
                     ' order by EstR08.dtehis, EstR08.seqhis ';
  EstR08.Open;
...
procedure TfmEstR08.OrdernacaopadraoClick(Sender: TObject);
var i: integer;
begin
  inherited;

  grR08DTEHIS.Sorted  := csNone;
  grR08OPERACAO.Sorted:= csNone;
  grR08QTDENT.Sorted  := csNone;
  grR08QTDSDA.Sorted  := csNone;
  grR08SLDATU.Sorted  := csNone;
