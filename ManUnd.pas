...
type
  TfmManUnd = class(TfmPadrao)
    ...
    procedure nvUndSalva(Sender: TObject);
    procedure EstUndBeforeDelete(DataSet: TDataSet);
  private
...
uses FPreview, Bbgeral, Bbmensag, ManPri, ManGDB;
...
procedure TfmManUnd.nvUndSalva(Sender: TObject);
begin
  inherited;
  ...
  try
     ...
     if fmManGDB.dbEmerion1.Connected then begin
        ...
        with quSQL2,SQL do begin
             ...
             Close;
             Text := ' Select * From EstUnd Where EstUnd.CodUnd = '''+ EstUndCodUnd.Value +'''';
             Open;

        end;

        if Trim( quSQL2.FieldbyName('CodUnd').AsString ) = '' then begin

           with quSQL2,SQL do begin

                Close;
                Text := ' Insert Into EstUnd(CodUnd,NomUnd) Values(:CodUnd,:NomUnd)';

                with Params do begin

                     Params[0].AsString := EstUndCodUnd.Value;
                     Params[1].AsString := EstUndNomUnd.Value;
           ...
           if EstUndNomUnd.Value <> quSQL2.FieldbyName('NomUnd').AsString then begin

              with quSQL2,SQL do begin

                   Close;
                   Text := ' Update EstUnd Set NomUnd = :NomUnd Where EstUnd.CodUnd = :CodUnd';

                   with Params do begin

                        Params[0].AsString := EstUndNomUnd.Value;
                        Params[1].AsString := EstUndCodUnd.Value;
     ...
     if fmManGDB.dbEmerion2.Connected then begin
        ...
        if Trim( quSQL3.FieldbyName('CodUnd').AsString ) = '' then begin
           ...
           if EstUndNomUnd.Value <> quSQL3.FieldbyName('NomUnd').AsString then begin
...
procedure TfmManUnd.EstUndBeforeDelete(DataSet: TDataSet);
begin
  inherited;
  ...
  try
     ...
     if fmManGDB.dbEmerion1.Connected then begin
        ...
        with quSQL2,SQL do begin
             ...
             Close;
             Text := ' Delete From EstUnd Where EstUnd.CodUnd = '''+ EstUndCodUnd.Value +'''';
             ExecSQL;
