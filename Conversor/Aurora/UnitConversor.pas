           ...
           if Query1.FieldByName('ESTOQ').AsFloat > 0 then begin
              ...
              if Length( Trim(Query1.FieldByName('SEQ').AsString) ) = 5 then begin
                 ...
                 if Trim( CodGru ) <> '' then begin
                    ...
                    quSql.Params.Clear;
                    quSql.SQL.Clear;
                    quSql.SQL.Add(' Update EstIte set Vb1Ite = :Vb1Ite, Vb2Ite = :Vb2Ite, Vb3Ite = :Vb3Ite, Flgtrg = :FlgTrg'+
                    ...
                    quSql.Params[0].Name     := 'Vb1Ite';
                    quSql.Params[0].DataType := ftFloat;
                    if (Query1.FieldByName('CUSTO').AsFloat > 0) and (Query1.FieldByName('T1').AsFloat > 0) then
                       quSql.Params[0].Value := Query1.FieldByName('CUSTO').AsFloat + fStrRound(fStrRound(Query1.FieldByName('CUSTO').AsFloat * Query1.FieldByName('T1').AsFloat,4)/100,4)
                    else
                       quSql.Params[0].Value := 0;

                    quSql.Params[1].Name     := 'Vb2Ite';
                    quSql.Params[1].DataType := ftFloat;
                    if (Query1.FieldByName('CUSTO').AsFloat > 0) and (Query1.FieldByName('T2').AsFloat > 0) then
                       quSql.Params[1].Value := Query1.FieldByName('CUSTO').AsFloat + fStrRound(fStrRound(Query1.FieldByName('CUSTO').AsFloat * Query1.FieldByName('T2').AsFloat,4)/100,4)
                    else
                       quSql.Params[1].Value := 0;

                    quSql.Params[2].Name     := 'Vb3Ite';
                    quSql.Params[2].DataType := ftFloat;
                    if Query1.FieldByName('CUSTO').AsFloat > 0 then
                       quSql.Params[2].Value := Query1.FieldByName('CUSTO').AsFloat
                    else
                       quSql.Params[2].Value := 0;
                       
                    quSql.Params[3].Name     := 'FlgTrg';
                    quSql.Params[3].DataType := ftString;
                    quSql.Params[3].Value    := '*';

                    quSql.Params[4].Name     := 'CodEmp';
                    quSql.Params[4].DataType := ftInteger;
                    quSql.Params[4].Value    := 5;
