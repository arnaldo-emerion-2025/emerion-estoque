...
type
  TfmManStr2 = class(TfmPadrao)
    ...
    BitBtn1: TBitBtn;
    pnMVADestSN: TPanel;
    edMVADestSN: TdxDBColorCurrencyEdit;
    lbMVADestSN: TLabel;
    dxDBColorCurrencyEdit5: TdxDBColorCurrencyEdit;
    Label33: TLabel;
    EstUfeMRGMVA_SN: TFloatField;
    EstUfeMRGMVA_SN_SINIEF: TFloatField;
    EstStrFLGATIVO: TSmallintField;
    ...
    procedure EdCodTxf1KeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
begin
  ...
  if (EdPsqTipStr.Text <> '') and (EdPsqTipStr.Text <> 'Todos') then
    sFiltro := sFiltro +  ' and  TipStr = ''' + EdPsqTipStr.Text + '''';
  ...
  begin
    ...
    if (edPsqStatus.ItemIndex > 0)then
    begin
        if(trim(edPsqStatus.Text) = 'Ativo')then
        begin
           sFiltro := sFiltro + ' and flgativo = 1 ';
        end
        else
           sFiltro := sFiltro + ' and flgativo = 0 ';
...
begin
  ...
  //Alteracao de regra de MVA - Pedido Carlos - 04/02/2020
  //ANTES
  //pnMVADestSN.Visible := (EstUfeSIGUFE.AsString = 'SC') or (EstUfeSIGUFE.AsString = 'PR') or (EstUfeSIGUFE.AsString = 'GO');

  //DEPOIS
  edMVADestSN.enabled := EstUfe.FieldByName('CodRegTrib').AsString <> '0';
  dxDBColorCurrencyEdit5.enabled := EstUfe.FieldByName('CodRegTrib').AsString <> '0';
end;
...
begin
  ...
  dest := 0;
  if      (EdSigUfe.Text = 'EX')               then dest := 3
  else if (EdSigUfe.Text = edUfEmitente.Text)  then dest := 1
  else if (EdSigUfe.Text <> edUfEmitente.Text) then dest := 2;
...
procedure TfmManStr2.EdCodTxf1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
   try
      if key = 112 then
         begin {F1 - Iniciais}
            ...
            fmAuxIni.TipoPesq := 'TEXTOFISCAL';
            fmAuxIni.ShowModal;
            if Trim(fmAuxIni.codTextFiscal) <> '' then
               begin
                  if not nvUfe.Salvar then
                     begin
                        if Trim(EstUfeUFEMITENTE.Value) <> '' then
                           EstUfe.Edit
                        else
                           EstUfe.Insert;
                     end;
                  EstUfeCODTXF.AsString := fmAuxIni.codTextFiscal;
               end;
         end
    finally
      FreeAndNil(fmAuxIni);
...
procedure TfmManStr2.ckStatusClick(Sender: TObject);
begin
  inherited;
        if(EstStr.State in [dsEdit])then
        begin
           if (ckstatus.Checked = false)then
           begin
               if (validaUsoStr(EstStrCodstr.AsString,EstStrTipSTR.AsString)) then
               begin
                   MessageBox(handle, 'Esta regra não pode ser inativada por estar em uso. Os produtos estão listados na ''Área de transferência''.','validação da regra de Substituição tributária',mb_ok+MB_ICONEXCLAMATION);
