...
begin
  ...
  if StrToIntDef(quSQL.FieldbyName('DECIMAIS_QTD').AsString,0) > 0 then
  begin
    EstSdfQTDSDF.DisplayFormat := '###,###,##0.0000';
    EstSdfQTDSDF.Precision     := quSQL.FieldbyName('DECIMAIS_QTD').AsInteger;
    EdQtdSdf.DisplayFormat     := '###,###,##0.0000';
    EdQtdSdf.DecimalPlaces     := quSQL.FieldbyName('DECIMAIS_QTD').AsInteger;
  end
  else
  begin
    EstSdfQTDSDF.DisplayFormat := '###,###,##0';
    EstSdfQTDSDF.Precision     := 0;
    EdQtdSdf.DisplayFormat     := '###,###,##0';
    EdQtdSdf.DecimalPlaces     := 0;
  end;
  ...
  if (EstSdfCodEmp.Value > 0) and
     ...
     with quSql,SQL do begin
          ...
          Text := 'Select qte.codemp,' +
                  ...
                  ' From EstQte qte' +
                  ' Where qte.CodEmp = '+ QuotedStr(IntToStr(EstSdfCodEmp.Value)) +
                  '   and qte.CodClp = '+ QuotedStr(EstSdfCodClp.Value) +
                  '   and qte.CodGru = '+ QuotedStr(EstSdfCodGru.Value) +
                  '   and qte.CodSub = '+ QuotedStr(EstSdfCodSub.Value) +
                  '   and qte.CodPro = '+ QuotedStr(EstSdfCodPro.Value) ;
...
procedure TfmManSda.FormKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;

  if ((key = #2) and (dsSdf.State in [dsInsert, dsEdit])) then
    begin

      EstSdf.FieldByName('CODCLP').AsString := '';
      EstSdf.FieldByName('CODGRU').AsString := '';
      EstSdf.FieldByName('CODSUB').AsString := '';
      EstSdf.FieldByName('CODPRO').AsString := '';
      ...
      try
        ...
        EstSdf.FieldByName('CODCLP').AsString := fmPsqBar.sCodClp;
        EstSdf.FieldByName('CODGRU').AsString := fmPsqBar.sCodGru;
        EstSdf.FieldByName('CODSUB').AsString := fmPsqBar.sCodSub;
        EstSdf.FieldByName('CODPRO').AsString := fmPsqBar.sCodPro;
