...
procedure TfmManStr.carregaCheckBox();
var
  grupos: string;
...
procedure TfmManStr.localiza(regra: string; entSai: string; estado: string = ''; regime: string = '');
var
  parametro: Variant;
begin
  if regra <> '' then
...
begin
  ...
  begin
    if pos('Where', sFiltro) = 0 then
      sFiltro := ' Where CodStr like ' + QuotedStr('%' + EdPsqCodStr.Text + '%')
    else
      sFiltro := sFiltro + ' and CodStr like ' + QuotedStr('%' + EdPsqCodStr.Text + '%');
  end;
...
begin
  ...
  if trim(EstStrTIPSTR.AsString) = 'Saida' then
    lbOrigemDestino.Caption := 'UF de Destino:'
  else
    lbOrigemDestino.Caption := 'UF de Origem:';
end;
...
begin
  ...
  btnLog.Visible := (GGus_Id = 1) or (GUsu_Id = 999);
end;

procedure TfmManStr.pEstUfeExit(Sender: TObject);
var
  Operac: string;
begin
  inherited;
...
procedure TfmManStr.nvUfeSalva(Sender: TObject);
begin
  inherited;
  ValidacaoUFE;
...
procedure TfmManStr.EstStrAfterPost(DataSet: TDataSet);
begin
  inherited;
  if not EdIcmSub1.Enabled then
  begin
...
procedure TfmManStr.edNCMKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure TfmManStr.ValidacaoSTR;
var
  strSql: string;
  RegAlt: integer;
  SQLTEMP: TQuery;
...
procedure TfmManStr.EstStrAfterScroll(DataSet: TDataSet);
var
  tipo: string;
begin
  inherited;
...
procedure TfmManStr.BbCalc_MVAClick(Sender: TObject);
begin
  inherited;
...
procedure TfmManStr.EstStrBeforePost(DataSet: TDataSet);
begin
  inherited;
  ValidacaoSTR;
end;

procedure TfmManStr.ValidacaoUFE;
var
  erros: string;
begin
...
procedure TfmManStr.EstUfeBeforePost(DataSet: TDataSet);
var
...
procedure TfmManStr.EstUfeAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmManStr.EdRegTribChange(Sender: TObject);
begin
  inherited;
...
procedure TfmManStr.CbCalc_MVAClick(Sender: TObject);
begin
  inherited;
...
procedure TfmManStr.EstUfeBeforeInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmManStr.EstUfeAfterInsert(DataSet: TDataSet);
var
  tipo: string;
begin
  inherited;
...
procedure TfmManStr.EstUfeAfterPost(DataSet: TDataSet);
begin
  inherited;
  edUfEmitente.Enabled := False;
  calcula;
...
procedure TfmManStr.EdSigUfeExit(Sender: TObject);
begin
  inherited;
...
procedure TfmManStr.EstUfeAfterScroll(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmManStr.btnAplicarClick(Sender: TObject);
begin
  inherited;
...
procedure TfmManStr.EstUfeSIGUFEChange(Sender: TField);
begin
  inherited;
  if ((EstUfe.FieldByName('CodRegTrib').AsString = '3') and (EstUfe.FieldByName('SIGUFE').Text <> 'MT')) then
    CbCalc_MVA.Enabled := True
...
procedure TfmManStr.EdCodCfo1Exit(Sender: TObject);
var
  dest: Integer;
  cfop: string;
  erro: boolean;
...
procedure TfmManStr.edRedSubChange(Sender: TObject);
begin
  inherited;
  if Trim(edRedSub.Text) <> '' then
    if StrToFloat(edRedSub.Text) > 0 then
...
procedure TfmManStr.btnLogClick(Sender: TObject);
begin
  inherited;
  try
    begin
      fmLogST := TfmLogST.Create(Self, EstStr.FieldByName('CODSTR').AsString,
        EstStr.FieldByName('TIPSTR').AsString);
      fmLogST.ShowModal;
    end
  finally
    FreeAndNil(fmLogST);
