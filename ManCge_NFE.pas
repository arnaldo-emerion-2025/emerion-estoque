...
uses
  ...
  StdCtrls, ComCtrls, ExtCtrls, DBCtrls, Db, Wwdatsrc, DBTables, 
...
uses  FileCtrl, Bbmensag, Bbgeral, Bbfuncao, ManGDB, ConCn1_NFE;
...
begin
  ...
  if Trim(FatGerObcGer.Value) <> '' then begin

     if Length(Trim(FatGerObcGer.Value)) >= 15 then begin

        sText := 'Deseja realmente cancelar a nota fiscal ?';

        if FatGerAtuEst.Value = 'Sim' then sEstoque := ' . Entrada em estoque dos itens;';

        if FatGerIntFin.Value = 'Sim' then sReceber := ' . Cancelamento do(s) titulo(s) a receber.';
        ...
        if fmsgConf(sText,'E') = 'SIM' then begin
           ...
           if fmsgConf('Tem certeza que esta realmente cancelando a nota fiscal correta ?','E') = 'SIM' then begin
              ...
              if sContinuar = 'S' then begin
                 ...
                 if FatGerFlgNfe.Value = 'Sim' then begin
