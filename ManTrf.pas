...
begin
  ...
  try
    ...
    if Trim(fmPsqTam.CodTam) <> '' then
      EdCodTam.Text := fmPsqTam.CodTam;
...
begin
  ...
  try
    ...
    if Trim(fmPsqEco.CodCor) <> '' then
      EdCodCor.Text := fmPsqEco.CodCor;
...
begin
  ...
  begin
    ...
    if EdValOpe.Value = 0 then
      fmsgErro('Campo de Preenchimento Obrigatorio não Informado. Valor.', EdValOpe);
    ...
    if (Trim(EdCodEmp1.Text) <> '') and
      ...
      (Trim(EdCodCor.Text) <> '') then
    begin

      if Trim(EdCodEmp1.Text) <> Trim(EdCodEmp2.Text) then
      begin

        if Trim(EdObsEnf.Text) <> '' then
        begin
          ...
          begin
            ...
            begin
              ...
              if sFlgTab = 'Ultimo Preco' then
                vbaIte := FieldbyName('CstIte').AsFloat;
              if sFlgTab = 'Custo Historico' then
                vbaIte := FieldbyName('VchIte').AsFloat;
              if sFlgTab = 'Custo Ponderado' then
                vbaIte := FieldbyName('VcpIte').AsFloat;
              if sFlgTab = 'Custo Referencial' then
                vbaIte := FieldbyName('VcrIte').AsFloat;
              if sFlgTab = 'Custo Historico (G)' then
                vbaIte := FieldbyName('VreIte').AsFloat;
              if sFlgTab = 'Custo Ponderado (G)' then
                vbaIte := FieldbyName('VprIte').AsFloat;
              if sFlgTab = 'Media Ponderada (E)' then
                vbaIte := FieldbyName('VmeIte').AsFloat;
              if sFlgTab = 'Custo da Ultima Compra' then
                vbaIte := FieldbyName('VpfIte').AsFloat;

              if EdValOpe.Value <> vbaite then
              begin

                if fmsgConf('Valor Informado para Entrada Diferente do Valor de Custo Informado para o Item de ' + FormatFloat('###,###,##0.00', vbaite) + '.'
                  + #10 + 'Confirma Alteração ? ', 'E') <> 'SIM' then
                begin
          ...
          begin
            ...
            begin
              ...
              if fMsg('Confirma Operação ?', 'O') then
              begin
...
begin
  ...
  begin
    ...
    if FieldbyName('FlgTam').AsString = 'Nao' then
    begin
    ...
    if FieldbyName('TamCor').AsString = 'Nao' then
    begin
