...
uses
  ...
  dxDBColorLookupEdit, dxDBColorEdit, dxColorDateEdit, dxColorEdit, PsqBar;
...
begin
  ...
  quSQL.Close;
  quSQL.SQL.Text := 'select coalesce(DECIMAIS_QTD, "N") as DECIMAIS_QTD From EstPar';
  quSQL.Open;

  if quSQL.FieldbyName('DECIMAIS_QTD').AsString = 'S' then
  begin
    EstEnfQTDENF.DisplayFormat := '##,###,##0.0000';
    EstEnfQTDENF.Precision     := 4;
    EdQtdEnf.DisplayFormat     := '##,###,##0.0000';
    EdQtdEnf.DecimalPlaces     := 4;
  end
  else
  begin
    EstEnfQTDENF.DisplayFormat := '###,###,##0';
    EstEnfQTDENF.Precision     := 0;
    EdQtdEnf.DisplayFormat     := '##,###,##0';
    EdQtdEnf.DecimalPlaces     := 0;
  end;
...
begin
  ...
  if (EstEnfCodEmp.Value > 0) and
     ...
     with quSql,SQL do begin
          ...
          Text := 'Select qte.codemp,' +
                  '       qte.QtaQte as QTAITE,' +
                  '       qte.QtdQte as QTDITE,' +
                  '       (SELECT QTD_RESERVADA FROM RESERVAS(QTE.CODEMP,QTE.CODCLP,QTE.CODGRU,QTE.CODSUB,QTE.CODPRO)) as QTRITE,' +
                  '       qte.QtdRma,' +
                  '       Round(qte.QtdQte-((SELECT QTD_RESERVADA FROM RESERVAS(QTE.CODEMP,QTE.CODCLP,QTE.CODGRU,QTE.CODSUB,QTE.CODPRO))+qte.QtdRma),4) as SLDITE' +
                  ' From EstQte qte' +
                  ' Where qte.CodEmp = '+ QuotedStr(IntToStr(EstEnfCodEmp.Value)) +
                  '   and qte.CodClp = '+ QuotedStr(EstEnfCodClp.Value) +
                  '   and qte.CodGru = '+ QuotedStr(EstEnfCodGru.Value) +
                  '   and qte.CodSub = '+ QuotedStr(EstEnfCodSub.Value) +
                  '   and qte.CodPro = '+ QuotedStr(EstEnfCodPro.Value) ;
          Open;

     end;

     EdQtdIte.Caption := FormatFloat(EstEnfQTDENF.DisplayFormat, quSQL.FieldbyName('QtdIte').AsFloat)+' ';
     EdQtrIte.Caption := FormatFloat(EstEnfQTDENF.DisplayFormat, quSQL.FieldbyName('QtrIte').AsFloat)+' ';
     EdQtdRma.Caption := FormatFloat(EstEnfQTDENF.DisplayFormat, quSQL.FieldbyName('QtdRma').AsFloat)+' ';
     EdQtaIte.Caption := FormatFloat(EstEnfQTDENF.DisplayFormat, quSQL.FieldbyName('QtaIte').AsFloat)+' ';
     ...
     EdSldIte.Caption := FormatFloat(EstEnfQTDENF.DisplayFormat, fRound(quSQL.FieldbyName('QtdIte').AsFloat - (quSQL.FieldbyName('QtrIte').AsFloat + quSQL.FieldbyName('QtdRma').AsFloat),4))+' ';
...
procedure TfmManEnf.FormKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;

  if ((key = #2) and (dsEnf.State in [dsInsert, dsEdit])) then
    begin

      EstEnf.FieldByName('CODCLP').AsString := '';
      EstEnf.FieldByName('CODGRU').AsString := '';
      EstEnf.FieldByName('CODSUB').AsString := '';
      EstEnf.FieldByName('CODPRO').AsString := '';
      ...
      try
        ...
        EstEnf.FieldByName('CODCLP').AsString := fmPsqBar.sCodClp;
        EstEnf.FieldByName('CODGRU').AsString := fmPsqBar.sCodGru;
        EstEnf.FieldByName('CODSUB').AsString := fmPsqBar.sCodSub;
        EstEnf.FieldByName('CODPRO').AsString := fmPsqBar.sCodPro;
