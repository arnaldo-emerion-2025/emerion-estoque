...
type
  ...
  TfmEstR01 = class(TfmPadrao)
    ...
    EstR01MK1ITE: TFloatField;
    EstR01MK2ITE: TFloatField;
    EstR01MK3ITE: TFloatField;
    EstR01MK4ITE: TFloatField;
    EstR01MK5ITE: TFloatField;
    grR01MK1ITE: TdxDBGridMaskColumn;
    grR01MK2ITE: TdxDBGridMaskColumn;
    grR01MK3ITE: TdxDBGridMaskColumn;
    grR01MK4ITE: TdxDBGridMaskColumn;
    grR01MK5ITE: TdxDBGridMaskColumn;
...
begin
  ...
  sBase := ' Select EstPro.DscPro,' +
    ...
    ' EstCor.NomCor,' +
    ' TextoOcor(EstPro.CodGru,' + QuotedStr('-') + ',EstGru.NomGru,' + QuotedStr('') + ',' + QuotedStr('') + ',' + QuotedStr('') + ',' + QuotedStr('') +
      ') as NomGru,' +
    ' TextoOcor(EstPro.CodSub,' + QuotedStr('-') + ',EstSub.NomSub,' + QuotedStr('') + ',' + QuotedStr('') + ',' + QuotedStr('') + ',' + QuotedStr('') +
      ') as NomSub,' +
    ' EstQte.CodClp || _UNICODE_FSS ' + QuotedStr('-') + ' || EstQte.CodGru || _UNICODE_FSS ' + QuotedStr('.') + ' || EstQte.CodSub || _UNICODE_FSS ' +
      QuotedStr('.') + ' || EstQte.CodPro as CodIte' +
...
begin
  ...
  if Trim(EdCodEmp.Text) <> '' then
    sFiltro := sFiltro + ' and EstQte.CodEmp = ''' + EdCodEmp.Text + '''';
  if Trim(EdCodClp.Text) <> '' then
    sFiltro := sFiltro + ' and EstQte.CodClp = ''' + EdCodClp.Text + '''';
  if Trim(EdCodGru.Text) <> '' then
    sFiltro := sFiltro + ' and EstQte.CodGru = ''' + EdCodGru.Text + '''';
  if Trim(EdCodSub.Text) <> '' then
    sFiltro := sFiltro + ' and EstQte.CodSub = ''' + EdCodSub.Text + '''';
  if Trim(EdCodPro.Text) <> '' then
    sFiltro := sFiltro + ' and EstQte.CodPro = ''' + EdCodPro.Text + '''';
