...
uses
  ...
  dxColorEdit, IBCustomDataSet, IBStoredProc, Mangdb;
...
type
  TfmManEmb = class(TfmPadrao)
    ...
    procedure EstEmbAfterPost(DataSet: TDataSet);
  private
...
begin
  ...
  if Trim(EdpsqCodEmb.Text) <> '' then
    sFiltro := ' Where CodEmb = ''' + EdpsqCodEmb.Text + '''';

  if Rgbusca.ItemIndex = 0 then
  begin

    if Trim(EdpsqNomEmb.Text) <> '' then
      sFiltro := ' Where NomEmb LIKE ''' + EdpsqNomEmb.Text + '%''';
...
procedure TfmManEmb.EstEmbAfterPost(DataSet: TDataSet);
begin
  inherited;
  FmManGdb.ReplicaTipoEmbalagem(EstEmbCODEMB.AsString);
end;
