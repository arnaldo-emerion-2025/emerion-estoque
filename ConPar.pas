...
procedure TfmConPar.PaintBoxPaint(Sender: TObject);
begin
  inherited;
  with Sender as TPaintBox do FillGrayGradientRect(PaintBox.Canvas, PaintBox.ClientRect, PaintBox.Color);
end;

procedure TfmConPar.FormShow(Sender: TObject);
begin
  inherited;
  EdPsqClsPro.SetFocus;
end;

procedure TfmConPar.bPesquisaClick(Sender: TObject);
begin
  inherited;
  if Trim( EdPsqClsPro.Text ) <> '' then begin
     ...
     if pos('Where',fmManPro.sFiltro) = 0 then
        fmManPro.sFiltro := ' Where EstPro.NumPro LIKE ''%'+ EdPsqClsPro.Text +'%'''
     else
        fmManPro.sFiltro := fmManPro.sFiltro + ' and EstPro.NumPro LIKE ''%'+ EdPsqClsPro.Text +'%''';
...
procedure TfmConPar.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
