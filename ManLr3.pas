...
begin
  ...
  begin
    if CmpNfsENVNFE.AsString = 'Nao' then
    begin
    ...
    begin
      ...
      begin
      strAux:= ExtractFilePath(Application.exename) + '\ECCe.exe ' + IfThen(trim(ParamStr(1)) <> '', ParamStr(1), 'DEFAULT@')
          + ' ' + CmpNfsSEQNFE.AsString + ' ' + 'CM' + ' ' + IntToStr(GUsu_Id) + ' CN PP';

        Executa(strAux, SW_NORMAL);
      end;
      CmpNfs.Close;
      CmpNfs.Open;
    end;
