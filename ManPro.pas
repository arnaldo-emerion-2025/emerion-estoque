...
uses
  ...
  Fpadrao,  ppBands, ppClass, ppVar, ppCtrls, ppPrnabl, ppProd,
  ...
  FileCtrl, dxDBColorPickEdit, clipbrd, dxDBColorHyperLinkEdit, dxDBColorMemo,
  dxDBColorDateEdit, dxDBColorCurrencyEdit, dxDBColorLookupEdit,
  dxDBColorEdit, dxColorEdit;
...
type
  TfmManPro = class(TfmPadrao)
    ...
    procedure nvSalva(Sender: TObject);
...
uses dxDemoUtils, ShellAPI, Bbacesso, BbGeral, Bbfuncao, BbMensag, ManGDB, PsqClp,
  ...
  ManBar, ConPar, Manpaf, Manthrpaf;
...
begin
  ...
  sBase := ' Select EstPro.*,' +
    '        EstPro.CodClp || _UNICODE_FSS ' + QuotedStr('-') + ' || EstPro.CodGru || _UNICODE_FSS ' + QuotedStr('.') + ' || EstPro.CodSub || _UNICODE_FSS ' +
...
begin
  ...
  begin
    ...
    if Trim(EdPsqId2Pro.Text) <> '' then
    begin

      if pos('Where', sFiltro) > 0 then
        sFiltro := sFiltro + ' and EstPro.Id2Pro LIKE ''' + EdPsqId2Pro.Text + '%'''
      else
        sFiltro := ' Where EstPro.Id2Pro LIKE ''' + EdPsqId2Pro.Text + '%''';
...
procedure TfmManPro.nvSalva(Sender: TObject);
var
  sCodBar: integer;
  CodBar: string;
  verificador: string;
...
begin
  ...
  begin
    ...
    Close;
    Text := ' Select Count(1) as Reg From EstPro ' + sFiltro;
    Open;
    ...
    begin
      ...
      begin
        ...
        if (EstPro_02DscPro.Value <> EstProDscPro.Value) or
          ...
          (EstPro_02CodBar.Value <> EstProCodBar.Value) then
        begin
    ...
    begin
      ...
      begin
        ...
        if (EstPro_03DscPro.Value <> EstProDscPro.Value) or
          ...
          (EstPro_03CodBar.Value <> EstProCodBar.Value) then
        begin
...
begin
  inherited;
  if nvQte.Salvar then
  begin

    if EstQte.State = dsInsert then
      Operac := 'dsInsert';
...
begin
  inherited;
  if nvTam.Salvar then
  begin

    if EstTam.State = dsInsert then
      Operac := 'dsInsert';
    ...
    begin
      ...
      if (Operac = 'dsInsert') and (not nvTam.Salvar) then
      begin

        if EstParFlgTam.Value = 'Sim' then
        begin
...
procedure TfmManPro.nvIteSalva(Sender: TObject);
begin
  inherited;
...
procedure TfmManPro.EstPro_02BeforeOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmManPro.EstPro_02TBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  if fmmanpri.Emerion_01 = false then
    sysutils.abort;
...
procedure TfmManPro.quSQL2BeforeOpen(DataSet: TDataSet);
begin
  inherited;
  if fmmanpri.Emerion_01 = false then
    sysutils.abort;
...
procedure TfmManPro.EstPro_03BeforeOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmManPro.quSQL3BeforeOpen(DataSet: TDataSet);
begin
  inherited;
  if fmmanpri.Emerion_02 = false then
    sysutils.abort;
...
procedure TfmManPro.ValidaDadosItem;
begin
...
procedure TfmManPro.EdIcmSaiChange(Sender: TObject);
begin
  inherited;
...
procedure TfmManPro.EdIcmEntChange(Sender: TObject);
begin
  inherited;
...
procedure TfmManPro.EdIpiSaiChange(Sender: TObject);
begin
  inherited;
...
procedure TfmManPro.EdIpiEntChange(Sender: TObject);
begin
  inherited;
...
procedure TfmManPro.EstProAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmManPro.EstProAfterRefresh(DataSet: TDataSet);
begin
  inherited;
  EdIcmSaiChange(EdIcmSai);
  EdIcmEntChange(EdIcmEnt);
...
procedure TfmManPro.EstProAfterScroll(DataSet: TDataSet);
begin
  inherited;
  EdIcmSaiChange(EdIcmSai);
  EdIcmEntChange(EdIcmEnt);
...
procedure TfmManPro.ValidacaoSubstituicao(CodStr, TipStr: string);
begin
...
procedure TfmManPro.EdCbaProKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if not (key in ['0'..'9']) then
    key := #0;
